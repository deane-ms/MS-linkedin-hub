...
- **Single external link fields** (one URL, not a list): bind the `<input>` directly to the field, no Add button, and render a `.notes-link-chip` anchor below it once non-empty (see `f-asset-folder` / `renderPostAssetFolderLink()` — the Google Drive asset-folder link on Posts). Contrast with Image links above, which is a *list* of URLs and needs the Add/remove chip pattern instead.
- **Status-as-pill select**: `#f-status` is a real `<select>` (native, accessible, no custom widget) but is skinned to look like the `.status-pill` badges shown on calendar cards — rounded, bold, colored per value. A `status-draft`/`status-approved`/`status-scheduled` class is kept in sync with the current value via `updateStatusSelectStyle()`, called on `change` and whenever the value is set programmatically (modal open/reset). If you add a similar "select that should look like its value" elsewhere, follow this pattern rather than building a custom dropdown.
