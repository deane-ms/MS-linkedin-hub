...
// on sign-in -- see upsertUserDirectory in content-hub-firebase.html), and emails them
// through Resend.
//
  ...
  for (const docSnap of pending.docs) {
    ...
    try {
      ...
      if (!email) {
        ...
        const itemLabel = notif.itemTitle || notif.taskName || "";
        const subject = notif.type === "mention"
          ? `${notif.author} mentioned you${itemLabel ? " in " + itemLabel : ""}`
          : `${notif.author} left feedback${itemLabel ? " on " + itemLabel : ""}`;
