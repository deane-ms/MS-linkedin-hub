#!/usr/bin/env node
// Syntax-checks every inline <script> block in the canonical source AND the generated index.html.
//
// Why this exists: the app is one big `<script type="module">` with no build step, so a single
// syntax error anywhere kills the entire page -- not just the feature that introduced it. The
// sibling Flowboard repo shipped exactly that (a literal `&amp;&amp;` in its module script) and
// the live board was dead for days before anyone noticed, because nothing about the deployed HTML
// looks wrong. Only an actual parse catches it.
//
// This repo is the more exposed of the two: `sync_from_scratchpad.py` does blind `str.replace()`
// into the HTML to produce index.html, and DESIGN.md records that mechanism silently breaking the
// build-version guard once already. So both files are checked by default -- the source you edit
// and the file that actually deploys.
//
...
const files = targets.length ? targets : ['content-hub-firebase.html', 'index.html'];
